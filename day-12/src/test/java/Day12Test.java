...
class Day12Test {
	...
	public static int[] dijkstrasShortestPath(ElfMap m, int src) {
		...
		for (int i = 0; i < m.numOfvertices; i++) {
				...
				if (!visited[j])// shortest distance of the node j should not have been finalized
				{
					...
					if (m.adjMatrix[closestVertex][j] != 0) {
						...
						if (d < distance[j]) { // distance via closestVertex is less than the initial distance
							// System.out.println(m.getValueAt(closestVertex)+" -> "+m.getValueAt(j)+" =
							// "+m.adjMatrix[closestVertex][j] );
							distance[j] = d;
						}
					}
				}
			...
			if(m.numOfvertices < 50) {
				m.printState(distance);
			}
		}
		return distance;
	}
	...
	class ElfMap {
		...
		ElfMap(List<String> input) {
			...
			for (int i = 0; i < input.size(); i++) {
				for (int j = 0; j < input.get(0).length(); j++) {
					...
					// north
					if (i + 1 < input.size()) {
						var heightNorth = getValueAt(i + 1, j);
					...
					// south
					if (i - 1 >= 0) {
						var heightSouth = getValueAt(i - 1, j);
					...
					if (j + 1 < input.get(i).length()) {
						var heightEast = getValueAt(i, j + 1);
					...
					// west
					if (j - 1 >= 0) {
						var heightWest = getValueAt(i, j - 1);
		...
		private void printState(int[] distance) {
			for (int i = 0; i < distance.length; i++) {
	...
	void part2(Stream<String> input, String solution) {
		...
		var result = Arrays.stream(startIndex).parallel()
				.map(i -> dijkstrasShortestPath(m, i)[m.getEndIndex()])
				.min();
